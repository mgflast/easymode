...
def fftshift_3d(x):
    return tf.roll(tf.roll(tf.roll(x, x.shape[-3]//2, axis=-3),
                          x.shape[-2]//2, axis=-2),
                  x.shape[-1]//2, axis=-1)
...
def DDWLoss(y_true, y_pred, mask, mask_rot):

    difference_map = y_true - y_pred
    fourier_difference = tf.signal.fft3d(tf.cast(difference_map, tf.complex64))

    denoising_term = mask * mask_rot * fourier_difference
    dewedging_term = 2 * (1 - mask) * mask_rot * fourier_difference

    denoising_term = tf.reduce_mean(tf.square(tf.abs(tf.signal.ifft3d(denoising_term))))
...
def train_denoiser(mode='splits', batch_size=32, box_size=96, epochs=100, lr_start=1e-3, lr_end=1e-5):
    ...
    # data loaders
    training_ds, training_steps = DDWDataLoader(mode=mode, batch_size=batch_size, box_size=box_size, validation=False).as_generator(batch_size=batch_size)
    validation_ds, validation_steps = DDWDataLoader(mode=mode, batch_size=batch_size, box_size=box_size, validation=True).as_generator(batch_size=batch_size)

...
def _denoise_tomogram_instance(volume, model, batch_size):
    ...
    segmented_tiles = _denoise_tile_list(tiles, model, batch_size=batch_size, max_chunk_size=MAX_CHUNK_SIZE)
...
def dispatch_segment(input_directory, output_directory, mode='splits', tta=1, gpus=(0,1,2,3), batch_size=8, overwrite=False, iter=1):
    ...
    print(f'Found {len(tomograms)} tomograms to segment in {input_directory}.')

    model_path = cache_model('denoiser_splits' if mode=='splits' else 'denoiser_direct')

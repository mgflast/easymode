...
`easymode denoise --method ddw`

A distilled [DeepDeWedge](https://doi.org/10.1038/s41467-024-51438-y) network. Separate DDW teacher networks were trained on each of 48 half-split datasets and then distilled into a single student network, so that the final network is general and can be applied directly to raw tomograms. Trained with Warp's tomogram reconstruction method.

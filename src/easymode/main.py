...
def main():
    ...
    segment.add_argument('--tta', required=False, type=int, default=2, help="Integer between 1 and 16. For values > 1, test-time augmentation is performed by averaging the predictions of several transformed versions of the input. Higher values can yield better results but increase computation time. (default: 2)")
    ...
    segment.add_argument('--tile', type=_parse_zyx, default=None, help="Inference tile size as ZxYxX (default 160x256x256). Each dim is capped at the volume size. Decrease if you run out of GPU memory.")
    segment.add_argument('--overlap', type=int, default=None, help="Overlap between neighbouring inference tiles, in voxels (default 32). Tiles stride by (tile - 2*overlap), so the overlap is reduced automatically if it is too large for the tile size.")
